...
class ProductionDefaultTests(unittest.TestCase):
    ...
    def test_demo_safe_profile_prefers_stability_and_evidence_collection(self) -> None:
        ...
        self.assertTrue(bool(cfg["fusion"]["require_speaking"]))
        self.assertTrue(bool(cfg["fusion"]["require_visual_speaking_for_visual_lock"]))
        ...
        self.assertEqual(float(cfg["fusion"]["thresholds"]["handoff_margin_min"]), 0.10)
    ...
    def test_pi_demo_live_profile_is_trimmed_for_single_speaker_stability(self) -> None:
        ...
        self.assertTrue(bool(cfg["fusion"]["audio_fallback"]["require_vad"]))
        ...
        self.assertNotIn("exposure_time_absolute", cfg["video"]["camera_controls"]["defaults"])
        ...
        self.assertNotIn("white_balance_temperature", cfg["video"]["camera_controls"]["defaults"])
        self.assertEqual(int(cfg["video"]["camera_controls"]["defaults"]["backlight_compensation"]), 1)
        self.assertEqual(int(cfg["vision"]["face"]["detect_every_n"]), 2)
        self.assertEqual(str(cfg["vision"]["mouth"]["backend"]), "diff")
        self.assertEqual(float(cfg["vision"]["track"]["jitter_deadband_px"]), 3.0)
        self.assertEqual(float(cfg["fusion"]["weights"]["audio"]), 0.35)
        self.assertEqual(float(cfg["fusion"]["thresholds"]["acquire_threshold"]), 0.24)
        self.assertEqual(int(cfg["fusion"]["thresholds"]["hold_ms"]), 1400)
        self.assertEqual(int(cfg["fusion"]["thresholds"]["handoff_min_ms"]), 1100)
        self.assertEqual(float(cfg["fusion"]["thresholds"]["handoff_margin_min"]), 0.10)
        self.assertEqual(int(cfg["fusion"]["thresholds"]["min_switch_interval_ms"]), 1000)
        self.assertEqual(float(cfg["fusion"]["thresholds"]["speak_on_threshold"]), 0.14)
    ...
    def test_pi_demo_measured_profile_uses_demo_lock_stabilization(self) -> None:
